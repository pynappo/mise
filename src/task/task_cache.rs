...
impl TaskArtifactCacheBuilder {
    ...
    pub(crate) async fn finish(self, ctx: TaskCacheContext<'_>) -> Result<TaskArtifactCache> {
        ...
        let remote = if mode == TaskCacheMode::LocalOnly {
            None
        } else if let Some(base_url) = settings.cache.remote_url.clone() {
            let namespace = settings
                ...
                .ok_or_else(|| {
                    eyre!("cache.remote_namespace is required when cache.remote_url is set")
                })?
                .to_string();
            Some(RemoteTaskCacheConfig {
                remote: RemoteCacheConfig {
                    base_url: base_url.parse().wrap_err("invalid cache.remote_url")?,
                    namespace,
                    token: settings.cache.remote_token.clone(),
                    token_file: settings.cache.remote_token_file.clone(),
                    oidc_audience: settings.cache.remote_oidc_audience.clone(),
                ...
                mode: settings.cache.remote_mode,
            })
        } else {
            None
        };
